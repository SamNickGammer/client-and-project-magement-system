    ...
    <Card className="hover:shadow-md transition-shadow">
      <CardHeader className="flex flex-row items-center gap-4 p-4 pb-2">
        ...
        <div className="flex flex-col overflow-hidden">
            ...
            {contact.position && <span className="text-sm text-muted-foreground truncate flex items-center gap-1"><Briefcase className="w-3 h-3"/> {contact.position}</span>}
        </div>
      </CardHeader>
      <CardContent className="p-4 pt-2 grid gap-2">
         ...
         {contact.email && (
            <div className="flex items-center gap-2 text-sm">
                ...
                <a href={`mailto:${contact.email}`} className="truncate hover:underline text-primary">{contact.email}</a>
            </div>
         )}
         {contact.phone && (
            <div className="flex items-center gap-2 text-sm">
                <Phone className="w-4 h-4 text-muted-foreground" />
                <a href={`tel:${contact.phone}`} className="truncate hover:underline text-primary">{contact.phone}</a>
            </div>
         )}
      </CardContent>
