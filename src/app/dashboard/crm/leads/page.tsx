...
export default function LeadsPage() {
  ...
  const handleConvert = async (lead: Lead) => {
    if (
      !confirm(
        `Convert "${lead.title}" to a Client? This will create a new Client and copy all contacts.`,
      )
    )
      return;
  ...
  const handleSubmit = async (
    data: Partial<Lead> & { contactIds?: string[] },
  ) => {
    try {
      ...
      if (!res.ok) {
        if (result.error && typeof result.error === "object") {
          const errors = Object.values(result.error.fieldErrors)
            .flat()
            .join(", ");
